    ...
    if request.method == "POST":
        ...
        if response.status_code == requests.codes['ok']:
            ...
            if opinions_count:
                ...
                plt.savefig(f"app/charts/{product_id}_recommendations.png")
...
@app.route('/products')
def products():
    if os.path.exists("app/products"):
...
def product(product_id):
    opinions = pd.read_json(f"app/opinions/{products_id}.json")
    return render_template("product.html", product_id=product_id, opinions=opinions_to_html(classes="table table-hover table-stripped", index=False))
    index = False, table_id

@app.route('/charts/<product_id>')
def product(product_id):
    return render_template("charts.html", product )
